 ...
 * @author luis.perez
 */
@Named("wpnClaveEspecialCrearIngresoClaveController")
@SessionScoped
public class ClaveEspecialCrearIngresoClaveController extends BDSUtil implements Serializable {
        ...
        if (validarAlfaNumerico(this.flujoExternaUsuarioDTO.getClaveOP())) {            
            if ((validarFormaClave(this.flujoExternaUsuarioDTO.getClaveOP(), "formIngresoDeClaveOpEspeciales:txtClave")) && (validarFormaClave(this.flujoExternaUsuarioDTO.getConfirmacionClaveOP(), "formIngresoDeClaveOpEspeciales:txtConfirClave"))) {
                ...
                if (!validarClavesIgualesCambClv()) {
                    ...
                    utilDTO = ibClavesOperacionesEspecialesDAO.insertarActualizarClaveOperacionesEspeciales(sesionController.getUsuario().getId().toString(), this.flujoExternaUsuarioDTO.getClaveOP(), sesionController.getUsuario().getId().toString(), true, sesionController.getIdCanal(), sesionController.getNombreCanal());
                    ...
                    } else if (ClavesOperacionesEspecialesEnum.YA_REGISTRADO.getDescripcion().equals(utilDTO.getResuladosDTO().get(ESTADO_CLAVE_OP))) {
                        FacesContext context = FacesContext.getCurrentInstance();
                        context.getExternalContext().getFlash().setKeepMessages(true);
                        FacesContext.getCurrentInstance().addMessage("formIngresoDeClaveOpEspeciales:txtConfirClave", new FacesMessage(FacesMessage.SEVERITY_ERROR, "", textosController.getNombreTexto("pnw.errores.texto.claves.OP.registrado", sesionController.getIdCanal())));
                        errores++;
                    }
                }
            } else {
            ...
            if (errores > 0) {
                this.redirectFacesContext("/sec/seguridad/clave_especial_cre_ingreso_datos.xhtml");
                return;
            }
            redirectFacesContext("/sec/seguridad/clave_especial_cre_ejecutar_pro.xhtml");
        }else {
            this.redirectFacesContext("/sec/seguridad/clave_especial_cre_ingreso_datos.xhtml");
            return;
        }
